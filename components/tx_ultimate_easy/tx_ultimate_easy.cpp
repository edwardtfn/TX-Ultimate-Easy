...
namespace esphome
{
    namespace tx_ultimate_easy
    {
        ...
        void TxUltimateEasy::handle_touch(int bytes[])
        {
            ESP_LOGV("UART-Log", "------------");
            for (int i = 0; i < 15; i++)
            {
        ...
        bool TxUltimateEasy::is_valid_data(int bytes[])
        {
            bool valid = true;
        ...
        int TxUltimateEasy::get_x_touch_position(int bytes[])
        {
            int state = bytes[4];
            switch (state)
            {
        ...
        int TxUltimateEasy::get_touch_state(int bytes[])
        {
            int state = bytes[4];
        ...
        TouchPoint TxUltimateEasy::get_touch_point(int bytes[])
        {
            TouchPoint tp;
